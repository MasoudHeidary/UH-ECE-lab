# -*- coding: utf-8 -*-
# File: transformer_bo/model_dense_enc.py
...
class Encoder(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor, connections_matrix: torch.Tensor) -> torch.Tensor:
        """
        Dynamic graph execution controlled by upper-tri adjacency (0..N).
        connections_matrix is (N+1)x(N+1) binary (torch.int/bool ok).
        """
        outputs = {0: x}
        num_layers = len(self.layers)

        conn = connections_matrix
        if not isinstance(conn, torch.Tensor):
            conn = torch.as_tensor(conn, device=x.device, dtype=torch.int)
        else:
            conn = conn.to(device=x.device, dtype=torch.int)
        ...
        for layer_idx, layer in enumerate(self.layers):
            ...
            for src_node in range(dest_node):
                on = conn[src_node, dest_node]
                on = int(on.item()) if isinstance(on, torch.Tensor) else int(on)
                if on != 0 and (src_node in outputs):
                    inputs_to_current.append(outputs[src_node])

            if inputs_to_current:
                y = layer.forward_dense(inputs_to_current, mask)
                outputs[dest_node] = y
...
class DecoderBlock(nn.Module):
    def __init__(self, features: int, self_attention_block: MultiHeadAttentionBlock,
        ...
        super().__init__()
        self.self_attention_block = self_attention_block
        self.cross_attention_block = self_attention_block if cross_attention_block is None else cross_attention_block
        self.feed_forward_block = feed_forward_block
...
def build_transformer(src_vocab_size: int, tgt_vocab_size: int,
    ...
    config = get_config()
    d_model = config['d_models'][0]      # your config uses a list
    N = config['num_layers']
    h = config['num_heads']
    ...
    for _ in range(N):
        ...
        encoder_blocks.append(EncoderBlock(d_model, attn, ffn, dropout))
    ...
    for _ in range(N):
        ...
        ffn = FeedForwardBlock(d_model, d_ff, dropout)
        decoder_blocks.append(DecoderBlock(d_model, self_attn, cross_attn, ffn, dropout))
