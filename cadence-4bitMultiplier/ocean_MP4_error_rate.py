...
def update_netlist(file_name: str, VA, VB, VBand, VBfa):
    _netlist = \
    f"""
...
V15 (pfa\<11\> gnd) vsource dc={VBfa[11]} type=dc
V14 (pfa\<10\> gnd) vsource dc={VBfa[10]} type=dc
V13 (pfa\<9\> gnd) vsource dc={VBfa[9]} type=dc
V12 (pfa\<8\> gnd) vsource dc={VBfa[8]} type=dc
V11 (pfa\<7\> gnd) vsource dc={VBfa[7]} type=dc
V10 (pfa\<6\> gnd) vsource dc={VBfa[6]} type=dc
V9 (pfa\<5\> gnd) vsource dc={VBfa[5]} type=dc
V8 (pfa\<4\> gnd) vsource dc={VBfa[4]} type=dc
V7 (pfa\<3\> gnd) vsource dc={VBfa[3]} type=dc
V6 (pfa\<2\> gnd) vsource dc={VBfa[2]} type=dc
V4 (pfa\<1\> gnd) vsource dc={VBfa[1]} type=dc
V3 (pfa\<0\> gnd) vsource dc={VBfa[0]} type=dc
V0 (vdd 0) vsource dc=800.0m type=dc
V1 (gnd 0) vsource dc=0 type=dc
V2 (pand gnd) vsource dc={VBand} type=dc
V100 (net4 gnd) vsource type=pulse val0=0 val1={'800m' if VA[0] else '0'} period=10n delay=100p \
        rise=1p fall=1p
V101 (net9 gnd) vsource type=pulse val0=0 val1={'800m' if VA[1] else '0'} period=10n delay=100p \
        rise=1p fall=1p
V102 (net14 gnd) vsource type=pulse val0=0 val1={'800m' if VA[2] else '0'} period=10n delay=100p \
        rise=1p fall=1p
V103 (net7 gnd) vsource type=pulse val0=0 val1={'800m' if VA[3] else '0'} period=10n delay=100p \
        rise=1p fall=1p
V104 (net12 gnd) vsource type=pulse val0=0 val1={'800m' if VB[0] else '0'} period=10n delay=100p \
        rise=1p fall=1p
V105 (net5 gnd) vsource type=pulse val0=0 val1={'800m' if VB[1] else '0'} period=10n delay=100p \
        rise=1p fall=1p
V106 (net11 gnd) vsource type=pulse val0=0 val1={'800m' if VB[2] else '0'} period=10n delay=100p \
        rise=1p fall=1p
V107 (net6 gnd) vsource type=pulse val0=0 val1={'800m' if VB[3] else '0'} period=10n delay=100p \
        rise=1p fall=1p
...
R22 (net6 B\<3\>) resistor r=0
R21 (net11 B\<2\>) resistor r=0
R20 (net5 B\<1\>) resistor r=0
R19 (net12 B\<0\>) resistor r=0
R18 (net7 A\<3\>) resistor r=0
R17 (net14 A\<2\>) resistor r=0
R16 (net9 A\<1\>) resistor r=0
R15 (net4 A\<0\>) resistor r=0
...
def update_ocean(file_name: str, log_name):
    script = \
    f"""
simulator( 'spectre )
...
plot(getData("/step") getData("/M<0>") getData("/M<1>") getData("/M<2>") getData("/M<3>") getData("/M<4>") getData("/M<5>") getData("/M<6>") getData("/M<7>") ?hcFontSize 18)
hardCopyOptions(?hcOutputFile "{log_name}.png" ?hcResolution 500 ?hcFontSize 18 ?hcOutputFormat "png" ?hcImageWidth 1920 ?hcImageHeight 1080)
hardCopy()

/*CALCULATE THE DELAYS FOR OUTPUTS*/
...
reportFile = outfile("{log_name}.txt")
...
for t_week in range(10, 100+1, 10):
    ...
    for VA in VA_lst:
        for VB in VB_lst:
            ...
            normal_log_name = f"./log/error-rate-normal-week{t_week:03}-A{''.join(map(str, VA))}-B{''.join(map(str, VB))}"
            update_netlist("/home/mheidary/simulation/main_MP4/spectre/schematic/netlist/netlist", VA, VB, VBand, VBfa)
            update_ocean("./tmp_main_MP4.ocn", normal_log_name)
            run_ocean_script("./tmp_main_MP4.ocn")
            log.println(normal_log_name)
            ...
            update_netlist("/home/mheidary/simulation/main_MP4/spectre/schematic/netlist/netlist", VA, VB, VBand, VBfa)
            update_ocean("./tmp_main_MP4.ocn", modify_log_name)
            run_ocean_script("./tmp_main_MP4.ocn")
            log.println(modify_log_name)
