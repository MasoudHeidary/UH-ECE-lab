...
log = Log(f"{__file__}.{BIT_LEN}.n{n_param}.log", terminal=True)
...
def get_error_rate(bitlen, vth, alpha, temp, sec, max_ps_delay):
    ...
    """the HTC attack"""
    for i,j in CRITICAL_FA_lst:
        fa_delay[i][j] *= (1 + n_param**2)
...
if True and __name__ == "__main__":
    log.println(f"RUNNING: error rate bitlen [{BIT_LEN}], REW_LST [HTH]")
