...
def get_monte_carlo_life_expect(alpha, vth_matrix, bit_len=bit_len):
    ...
    current_fail *= 0.90    # 10% more tolerable to failure current 
    # vb_fail = get_pb_from_current(current_fail)

    for t_week in range(300):
        ...
        for fa_i in range(bit_len - 1):
            for fa_j in range(bit_len):
                for t_index in range(6):
                    transistor_current = get_current_from_pb(body_voltage[fa_i][fa_j][t_index])
                    # if body_voltage[fa_i][fa_j][t_index] >= vb_fail:
                    if transistor_current <= current_fail:
                        return {
                            "fa_i": fa_i,
                            "fa_j": fa_j,
                            "t_index": t_index,
...
def monte_carlo_lifetime(
    ...
    for sample_index in range(sample_count):
        ...
        if log_obj:
            log_obj.println(f"SAMPLE[{sample_index:5}] lifetime[{lifetime:3} weeks], average[{sum_lifetime/(sample_index+1):6.2f}]")
